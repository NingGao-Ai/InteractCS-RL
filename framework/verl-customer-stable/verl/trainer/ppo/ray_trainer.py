    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Collect interaction metrics from rollout (stored in non_tensor_batch)
                # Extract interaction metrics (they start with "interaction/")
                for key in gen_batch_output.non_tensor_batch.keys():
                    if "interaction" in key:
                        # Take the first value since all values are the same (repeated scalar)
                        value = gen_batch_output.non_tensor_batch[key][0]
