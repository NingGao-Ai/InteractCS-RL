...
class SGLangRollout(BaseRollout):
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        # Finalize interaction and collect metrics
        if _req.interaction_kwargs and self.interaction_map:
            ...
            if interaction_name in self.interaction_map:
                ...
                # Collect interaction metrics for logging
                interaction_metrics = interaction.get_metrics_for_logging()
                if interaction_metrics:
                    for key, value in interaction_metrics.items():
                        _req.metrics[key] = value
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # Collect interaction metrics from all requests
        # Store as scalar values repeated for each sample in the batch
        interaction_metrics = {}
        for req in sorted_output_req_list:
            if req.metrics:
                for key, value in req.metrics.items():
                    interaction_metrics[key] = value  # Use the latest value
        
        # Add interaction metrics to non_tensor_batch as repeated scalar values
        if interaction_metrics:
            batch_size = len(sorted_output_req_list)
            for key, value in interaction_metrics.items():
                # Store as numpy array with repeated values for each sample
                non_tensor_batch[key] = np.full(batch_size, value, dtype=np.float32)
