...
Main features:
1. Global statistics collection (voucher rate, etc.)
...
class SharedStatsManager:
    ...
    def __init__(self, config: dict):
        ...
        self.voucher_cost_lambda = reward_config.get("initial_voucher_cost", 0)
        ...
        # EMA-smoothed observed rate (initialized to target value)
        self.smoothed_voucher_rate = config.get("resume_smoothed_voucher_rate", self.target_voucher_rate)
    ...
    def _update_cost_lambda(self):
        ...
        Strategy:
        1. Compute current voucher rate from all samples in the sliding window
        ...
        # ablation: no smooth
        # voucher_error = current_voucher_rate - self.target_voucher_rate
        ...
        # ablation: no smooth
        # self.voucher_cost_lambda = max(0.0, min(self.max_cost, voucher_error))
        # ablation: fix lambda
        # self.voucher_cost_lambda = 0.1
    ...
    def get_current_costs(self) -> Dict[str, float]:
        ...
        Returns:
            dict: Contains voucher_cost_lambda
        """
        with self._lock:
            return {
                "voucher_cost_lambda": self.voucher_cost_lambda,
    ...
    def get_metrics_for_logging(self) -> Dict:
        ...
        with self._lock:
            metrics = {
                ...
                "interaction/train_voucher_rate": 0,
                "interaction/smoothed_train_voucher_rate": 0,
                "interaction/train_avg_base_score": 0,
                "val-interaction/total_interactions": 0,
                "val-interaction/test_voucher_rate": 0,
                "val-interaction/test_window_size": 0,
            }
...
def get_or_create_shared_stats_manager(config: dict, force_create: bool = False) -> ray.actor.ActorHandle:
    ...
    target_namespace = "verl_training_customer_service"

    if not force_create:
        try:
            # Try to get existing actor
            manager = ray.get_actor(actor_name,namespace=target_namespace)
