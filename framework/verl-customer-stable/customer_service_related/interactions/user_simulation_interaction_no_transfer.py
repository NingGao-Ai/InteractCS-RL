...
from .shared_stats_manager_no_transfer import get_or_create_shared_stats_manager
import json
...
class UserSimulationInteraction(BaseInteraction):
    ...
    def __init__(self, config: dict):
        ...
        print(f"Initializing UserSimulationInteraction with config: {config}")
        ...
        print("============= Initializing shared stats manager =================")
        self.shared_stats_manager = get_or_create_shared_stats_manager(config)
        print("============= Shared stats manager initialized =================")
    ...
    async def generate_response(
        ...
        if assistant_message:
            ...
            action_match = re.search(r'<action>\s*(chat|voucher)\s*</action>', message_content, re.DOTALL | re.IGNORECASE)
            if action_match:
            ...
            '''
            tool_call_match = re.search(r'<action>\s*(.*?)\s*</action>', message_content, re.DOTALL | re.IGNORECASE)
            if tool_call_match:
            ...
            # Check tool name and extract action
                    if tool_data.get("name") == "customer_service":
                        action_type = tool_data.get("arguments", {}).get("action")
                        if action_type and action_type.lower() in ["chat", "voucher"]:
                            action_type = action_type.lower()
                    
                except (json.JSONDecodeError, AttributeError, KeyError):
                    # JSON parsing failed, try direct search
                    action_type = "null"
