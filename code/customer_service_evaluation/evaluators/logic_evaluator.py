"""
Logic Evaluator - Uses evaluation-logic.txt for evaluation
"""

import json
import os
from typing import Dict, List, Any, Optional
from core.evaluation_result import LogicEvaluationResult
...
class LogicEvaluator:
    ...
    def __init__(self, llm_config, prompt_file: str):
        ...
        Args:
            ...
            prompt_file: Logic evaluation prompt file path
        """
    ...
    def _load_prompt_template(self) -> str:
        ...
        try:
            ...
            raise Exception(f"Failed to load logic evaluation prompt file: {e}")

    def _format_dialogue_for_logic_evaluation(self, conversation_history: List[Dict[str, Any]]) -> str:
        """
        Format dialogue for logic evaluation

        Logic evaluation uses the complete user and assistant dialogue
        """
        dialogue_lines = []
        ...
        for turn in conversation_history:
            if turn['role'] == 'user':
                ...
                # Logic evaluation uses full content (agent perspective)
                dialogue_lines.append(f"assistant: {turn['content']}")

        return '\n'.join(dialogue_lines)

    def evaluate(self, conversation_history: List[Dict[str, Any]]) -> LogicEvaluationResult:
        """
        Evaluate dialogue logic quality

        Args:
            conversation_history: Conversation history

        Returns:
            LogicEvaluationResult: Logic evaluation result
        """
        try:
            # Format dialogue (using full dialogue)
            dialogue_text = self._format_dialogue_for_logic_evaluation(conversation_history)
            ...
            # Extract reasons
            reasons = {}
            categories = ['user_profile_recognition', 'business_rule_capability', 'ood_issues']
            ...
            # Create evaluation result object (compensation strategy excluded, moved to CompensationEvaluator)
            return LogicEvaluationResult(
                user_profile_recognition=evaluation_data.get('user_profile_recognition', {}),
                business_rule_capability=evaluation_data.get('business_rule_capability', {}),
                ood_issues=evaluation_data.get('ood_issues', {}),
                reasons=reasons
            )
